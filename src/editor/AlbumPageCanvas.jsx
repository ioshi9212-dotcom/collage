...
export const DEFAULT_ALBUM_PREVIEW_WIDTH = 520;
...
export function PreviewPageNumber({ pageIndex, canvas, settings }) {
...
export default function AlbumPageCanvas({ project, pageIndex, previewWidth = DEFAULT_ALBUM_PREVIEW_WIDTH }) {
  const canvas = project.canvas || { width: 1480, height: 2100 };
  const settings = project.settings || {};
  const page = project.pages?.[pageIndex];
  const safePreviewWidth = Math.max(240, Math.round(Number(previewWidth) || DEFAULT_ALBUM_PREVIEW_WIDTH));
  const height = Math.max(1, Math.round(safePreviewWidth * canvas.height / Math.max(1, canvas.width)));
  const scale = safePreviewWidth / Math.max(1, canvas.width);
  ...
  return (
    <Stage className="album-page-canvas album-flip-stage" width={safePreviewWidth} height={height} listening={false}>
