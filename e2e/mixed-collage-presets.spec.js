...
test('keeps the original mixed compositions and applies overlays', async ({ page }) => {
  ...
  await expect(picker.locator('.collage-preset-card')).toHaveCount(22);
...
test('keeps all 22 presets reachable and applies an A5-safe album layout on a phone', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  const picker = await openPresetPicker(page);
  await expect(picker.locator('.collage-preset-card')).toHaveCount(22);

  await picker.locator('.collage-preset-categories').getByRole('button', { name: 'Как в альбоме', exact: true }).click();
  await expect(picker.locator('.collage-preset-card')).toHaveCount(10);

  const newPreset = picker.locator('[data-preset-id="album-5-1"]');
  ...
  await expect.poll(() => currentPageState(page)).toMatchObject({
    settings: { frameCount: 5, frameMode: 'free' },
    page: { frameCount: 5, layout: null, collagePresetId: 'album-5-1' },
  });
});
